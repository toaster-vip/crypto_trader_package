...
CONFIG = {
    ...
    # --- 4 小时同频过滤（新增） ---
    "USE_4H_FILTER": True,            # 是否启用 4h 同频硬过滤
    "MIN_PCT_4H": 0.005,               # 4h 涨幅下限（+0.5%）
    "EMA_WINDOW_1H": 6,                # EMA 计算窗口（小时）
    "REQUIRE_LAST1H_ABOVE_EMA": True,  # 最后一根收盘需站上 EMA
    "MIN_VOL_FACTOR": 1.1,             # 近6根均量 / 全部均量 >= 1.1
    "RELAX_ON_FEW": True,              # 候选过少时是否自动放宽
    "RELAX_FACTOR": 0.8,               # 阈值宽松比例（0.8=放宽20%）
