...
def fetch_score(symbol):
    try:
        score_obj = get_symbol_score(symbol)
...
def main():
    api = KuCoinClient()
    hot_symbols = get_top_gainers_and_volume(top_n=CONFIG["HOT_TOP_N"])
    if not hot_symbols:
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=CONFIG["DEFAULT_WORKERS"]) as executor:
        futures = [executor.submit(fetch_score, s) for s in hot_symbols]
